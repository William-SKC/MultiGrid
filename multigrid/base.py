...
class MultiGridEnv(gym.Env, RandomMixin, ABC):
    ...
    def __init__(
        ...
        for agent_name in self.possible_agents:
            ...
            self.action_spaces[agent_name] = agent.action_space
        ...
        for agent_name in self.possible_agents:
            self.state_spaces[agent_name] = spaces.Box(
                low=0, high=255, shape=(self.height, self.width, WorldObj.dim), dtype=int)
    ...
    def reset(
        ...
        self.terminiation_step = {agent_index: -1 for agent_index in range(self.num_agents)}
    ...
    # def state(
    #     self):
    #     print("Multigird state() was called!")
    #     return torch.zeros(10)  # Dummy return to check if it’s used
